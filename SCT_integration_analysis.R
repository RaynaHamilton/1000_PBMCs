#this file isn't super necessary when only one group is present, but good practice
...
#seurat_phase$sample[which(str_detect(seurat_phase$cells,"^ctrl_"))] <- "ctrl"
#seurat_phase$sample[which(str_detect(seurat_phase$cells,"^stim_"))] <- "stim"
...
#integration doesn't mean anything when there's only 1 group
